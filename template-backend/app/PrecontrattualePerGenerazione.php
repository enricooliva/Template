...
class PrecontrattualePerGenerazione extends Precontrattuale {
    ...
    //$sesso = sesso($id_insegn); $this->anagrafica->sesso
    //$nameTutor = utf8_decode(codifica(nameTutor($tutorid))); $this->user->nameTutorString() 
    //$cognome = cognome($tutorid); //$this->user->cognome
    //$nome = nome($tutorid); //$this->user->nome
    //$cf = cf($tutorid); ////$this->user-cf
    //$email = email($tutorid); //$this->user->email
    ...
    {
        return $this->insegnamento->ruoloToString();
    }
    ...
    public function ContatoreSelection(){
        ...
        $counter = $this->insegnamento->contatore($this->user->cf); //contatore_insegnamenti($cod_insegnam, $cf); 
